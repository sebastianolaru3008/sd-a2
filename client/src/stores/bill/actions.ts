...
export const placeOrder = (customerId: string, items: FoodOrderItem[], restaurantId: string) : ThunkAction < void, RootState, null, AnyAction> => 
async (dispatch) =>{
    try{
        const data = {
          customerId: customerId,
          orderedFoods : items.map((foodItem) => ({foodId:foodItem.item.id, quantity: foodItem.quantity} as OrderedFoodDto)),
